<?php
/**
 * Security Trait for AS PHP Checkup
 *
...
/**
 * AS_PHP_Checkup_Security trait
 *
 * @since 1.2.1
 */
trait AS_PHP_Checkup_Security {
	 ...
	 * @var array
	 */
	protected $allowed_solution_types = array(
		'php_ini',
		'user_ini',
		'htaccess',
		'wp_config',
	 ...
	 * @var array
	 */
	protected $allowed_config_types = array(
		'php_ini',
		'user_ini',
		'htaccess',
		'nginx',
	...
	protected function validate_file_path( $file_path ) {
		...
		// Check for sensitive files that we're allowed to modify
		$allowed_files = array(
			'wp-config.php',
			'.htaccess',
			'php.ini',
		...
		// If it's a backup file, it should have .backup extension
		if ( strpos( $filename, '.backup' ) !== false ) {
			return true;
		}
		
		// Check if it's an allowed file for modification
		$is_allowed = false;
		...
		if ( ! $is_allowed ) {
			return new WP_Error(
				'file_not_allowed',
				__( 'File modification not allowed', 'as-php-checkup' )
			);
		}
	...
	public function cleanup_old_backups() {
		...
		foreach ( $cleanup_dirs as $directory ) {
			...
			if ( strpos( $directory, wp_normalize_path( ABSPATH ) ) !== 0 ) {
				continue;
			}
	...
	protected function log_operation( $operation, $data = array() ) {
		...
		if ( AS_PHP_CHECKUP_DEBUG && defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ) {
			error_log( sprintf(
				'[AS PHP Checkup] %s: %s by user %d',
				$operation,
				wp_json_encode( $data ),
				get_current_user_id()
			) );
	...
	private function validate_ini_syntax( $content ) {
		...
		foreach ( $dangerous as $directive ) {
			if ( isset( $parsed[ $directive ] ) && ! in_array( $directive, array( 'allow_url_fopen' ), true ) ) {
				return new WP_Error(
					'dangerous_directive',
					sprintf(
						/* translators: %s: directive name */
						__( 'Potentially dangerous directive detected: %s', 'as-php-checkup' ),
						$directive
					)
				);
			}
	...
	private function validate_nginx_syntax( $content ) {
		...
		if ( strpos( $content, 'location' ) === false && strpos( $content, 'fastcgi_param' ) === false ) {
			return new WP_Error(
				'nginx_missing_directive',
				__( 'Missing required directives in NGINX configuration', 'as-php-checkup' )
			);
		}
			...
			if ( stripos( $content, $func . '(' ) !== false ) {
				return new WP_Error(
					...
					sprintf(
						...
						__( 'Dangerous function detected: %s', 'as-php-checkup' ),
						$func
					)
				);
			}
